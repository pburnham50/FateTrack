...
#put your path here
input_path = sys.argv[1] #'~/Desktop/ml_data/'
input_path_seg = sys.argv[2] #'~/Desktop/ml_data/'
output_path = sys.argv[3] #'~/Desktop/ml_data/improps_0315/'

sample = sys.argv[4]

newnp = np.asarray(np.load(input_path_seg + 'dapi_' + sample + '_seg.npy',allow_pickle=True)).item()
outs = (newnp['outlines']>0).astype(int)
masks = newnp['masks']
imorig = newnp['img']

dapi = io.imread( input_path + 'dapi_' + sample + '.tif');
gfp = io.imread( input_path + 'gfp_' + sample + '.tif');
a594 = io.imread( input_path + 'a594_' + sample + '.tif');
cy3 = io.imread( input_path + 'cy3_' + sample + '.tif');
cy5 = io.imread( input_path + 'cy5_' + sample + '.tif');
...
writeChannelFeatures(gfp, "gfp", ringMask = ringmask)
...
export = im2_df.to_csv(output_path + "nuclei_" + sample + ".feature.csv", index = None, header=True)
