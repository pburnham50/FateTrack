...
from skimage import color, feature, filters, io, measure, morphology, restoration, segmentation, exposure, data
import warnings
import math
import pandas as pd
...
#put your path here
input_path = sys.argv[1] #'~/Desktop/ml_data/HCR_0611/507_D3_1_w1/subimages/'
input_path_seg = sys.argv[2] #'~/Desktop/ml_data/HCR_0611/507_D3_1_w1/subimages/'
output_path = sys.argv[3] #'~/Desktop/ml_data/HCR_0611/507_D3_1_w1/subimages/'
subimages = sys.argv[4] #5

sample_list = range(0, int(subimages) + 1)

#sample = sys.argv[4] #
for sample in sample_list:
     newnp = np.asarray(np.load(input_path_seg + 'sub_' + str(sample) + '_2DAPI_seg.npy',allow_pickle=True)).item()
     ...
     export = im2_df.to_csv(output_path + 'sub_' + str(sample) + "_nuclei.feature.csv", index = None, header=True)
